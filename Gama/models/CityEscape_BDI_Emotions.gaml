...
//	file shapefile_shelters <- file("../includes/Escapes.shp");
	...
	init {
		...
		create hazard {location<-any_location_in(world.shape);}//from: shapefile_hazard;
		create catastrophe;
		create shelter number:6 {location<-any_location_in(world.shape);}// from: shapefile_shelters;
...
species people skills: [moving] control: simple_bdi{
	...
	aspect default {
		draw triangle(90) rotate: heading + 90 color: color;
	}
}
