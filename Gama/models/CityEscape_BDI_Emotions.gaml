...
global {
	...
	image_file DH_bg0_image_file <- image_file("../includes/DH_bg.png");
	file shapefile_roads <- file("../includes/DH_roads.shp");
	...
	float hazard_distance <- 4.0;
	float catastrophe_distance <- 1.0;
	float proba_detect_hazard <- 0.02;
	float proba_detect_other_escape <- 0.01;
	float other_distance <- 1.0;
	...
	init {
		...
		} //from: shapefile_hazard;
		create catastrophe;
		create shelter number: 6 {
			location <- any_location_in(world.shape);
...
species people skills: [moving] control: simple_bdi {
	...
	aspect default {
		draw triangle(10) rotate: heading + 90 color: color depth: 10;
	}
...
species road {
	...
	float speed_coeff <- 1.0 update: exp(-nb_people * 3 / capacity) min: 0.1;
...
species water {
	geometry shape <- world;
	float dd <- 1.0;
	int sign <- 1;

	reflex {
		sign <- (dd > 6 or dd<1) ? -sign : sign;
		dd <- dd + sign * 0.01;
	}
...
//	float minimum_cycle_duration <- 0.02;
	output {
		...
		//			image DH_bg0_image_file  ;
			...
			species water;
			mesh terrain texture: DH_bg0_image_file scale: 1 triangulation: true color: palette([#burlywood, #saddlebrown, #darkgreen, #green]) refresh: false smooth: true;
		}
