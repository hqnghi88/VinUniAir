...
//Shapefile for the river
	...
	file dykes_shapefile <- file("../includes/roads1.shp");
	//Shapefile for the buildings
	file buildings_shapefile <- file("../includes/buildings.shp");
	file shape_file_evacuation <- file("../includes/evacuation_point.shp");

	//Data elevation file
	file dem_file <- grid_file("../includes/DEM.tif");
	//Diffusion rate
	//   float diffusion_rate <- 0.9;
	//Height of the dykes
	float dyke_height <- 25.0;
	//Width of the dyke
	float dyke_width <- 15.0;
	float diffusion_rate <- 0.01;
	float diffusion_rate0 <- 0.01;
	float diffusion_rate1 <- 0.01;
	float diffusion_rate2 <- 0.01;
	float diffusion_rate3 <- 0.01;
	float diffusion_rate4 <- 0.01;
	...
	//Initialization of the cells
		...
		do init_water;
		create sensor from: shape_file_evacuation {
			mycell <- cell at_distance 500;
			ask mycell {
				d_rate <- myself.diffusion_rate;
			}

		}
		//      ask cell{ec<-sensor closest_to self;}
	...
	action init_obstacles {
		...
		//      save dyke to:"../includes/roads1.shp" format:"shp";
		ask dyke {
			...
			//We compute the height of the neighbours cells according to their altitude, water_height and obstacle_height
				...
				if (!empty(flow_cells)) {
					loop flow_cell over: shuffle(flow_cells) sort_by (each.height) {
						float water_flowing <- max([0.0, min([(height - flow_cell.height), water_height * d_rate])]);
...
species sensor {
	...
	aspect base {
		draw pyramid(150) color: color;
		draw string(self) size: 20 at: {location.x - 100, location.y, 200} perspective: true;
	}
...
experiment Run type: gui {
	...
	parameter "Diffusion rate Overall" var: diffusion_rate category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		ask cell {
			d_rate <- diffusion_rate;
		}

	}

	parameter "Diffusion Sensor 0" var: diffusion_rate0 category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		sensor[0].diffusion_rate <- diffusion_rate0;
		ask sensor[0].mycell {
			d_rate <- diffusion_rate0;
		}

	}

	parameter "Diffusion Sensor 1" var: diffusion_rate1 category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		sensor[1].diffusion_rate <- diffusion_rate1;
		ask sensor[1].mycell {
			d_rate <- diffusion_rate0;
		}
	...
	parameter "Diffusion Sensor 2" var: diffusion_rate2 category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		sensor[2].diffusion_rate <- diffusion_rate2;
		ask sensor[2].mycell {
			d_rate <- diffusion_rate0;
		}
	...
	parameter "Diffusion Sensor 3" var: diffusion_rate3 category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		sensor[3].diffusion_rate <- diffusion_rate3;
		ask sensor[3].mycell {
			d_rate <- diffusion_rate0;
		}
	...
	parameter "Diffusion Sensor 4" var: diffusion_rate4 category: "Water dynamic" min: 0.01 max: 1.0 step: 0.01 {
		sensor[4].diffusion_rate <- diffusion_rate4;
		ask sensor[4].mycell {
			d_rate <- diffusion_rate0;
		}
	...
	//layout vertical([0::5000,1::5000]) tabs:false editors: false;
		display map type: 3d background: #black axes: false {
			species buildings aspect: geometry refresh: false;
			species dyke aspect: geometry;
			species sensor aspect: base;
			grid cell elevation: grid_value triangulation: true transparency: 0.5;
		}

		display chart_display refresh: every(24 #cycles) type: 2d background: #black {
			chart "Pressure on Dykes" background: #gray type: series legend_font: font("Helvetica", 18) label_font: font("Helvetica", 20, #bold) title_font: font("Helvetica", 24, #bold) {
