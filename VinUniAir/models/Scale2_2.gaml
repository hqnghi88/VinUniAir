...
global {
	...
	shape_file roads_shape_file <- shape_file("../includes/lvl2/gialam_roads.shp");
	//	shape_file dummy_roads_shape_file <- shape_file(resources_dir + "vinuniroad.shp");
	shape_file buildings_shape_file <- shape_file("../includes/lvl2/gialam_b.shp");
	...
	init {
		...
		[x::-3000, y::yy + 180 * HH, width::250 * WW, height::20 * HH, max_val::(max_cars + max_bus + max_motorbikes), title::lb_rates_EG, left_label::"0%", right_label::"100%", scale::HH];
		create progress_bar with: [x::-3000, y::yy + 260 * HH, width::250 * WW, height::20 * HH, max_val::max_cars, title::lb_cars, left_label::"0%", right_label::"100%", scale::HH];
		create progress_bar with:
		[x::7200, y::yy + 180 * HH, width::250 * WW, height::20 * HH, max_val::max_motorbikes, title::lb_motobike, left_label::"0%", right_label::"100%", scale::HH];
		create progress_bar with: [x::7200, y::yy + 260 * HH, width::250 * WW, height::20 * HH, max_val::max_bus, title::lb_bus, left_label::"0%", right_label::"100%", scale::HH];
		//end-Sliders
		...
		create line_graph_aqi with: [x::5900, y::ly + 30 * HH, width::300 * WW, height::110 * HH, label::"Hourly AQI", thick_axe::1, thick_line::5];
		...
		create param_indicator with: [x::WW * lx * xx_sc + xx, y::560 * HH, size::30, name:: lb_AQI_update, value:: "", with_box::false, width::200 * WW, height::20 * HH];
		create api_loader;
		ask api_loader {
...
//	}
	...
	output synchronized: false {
		...
		display main type: opengl background: #black axes: false {
			...
			camera 'default' location: {14151.3816, 15136.6783, 34335.7691} target: {14151.3816, 15136.079, 0.0}; 
			//			light #ambient intensity: 256;
			//
			//
			//
			image ("../includes/bigger_map/hanoi_dark.png") position: {5000+lx * WW * xx_sc + xx, 100 * HH, -0.01} size: {0.5, 0.5};
			species building aspect: border refresh: false position: {5000+lx * WW * xx_sc + xx, 100 * HH, 0} size: {0.5, 0.5};
			species traffic_incident position: {5000+lx * WW * xx_sc + xx, 100 * HH, 0.01} size: {0.5, 0.5};
			//
			//
			//
			//
			//
			image ("../includes/bigger_map/hanoi_dark.png") position: {5000+lx * WW * xx_sc + xx, 560 * HH, -0.01} size: {0.5, 0.5};
			species building aspect: border refresh: false position: {5000+lx * WW * xx_sc + xx, 560 * HH, 0} size: {0.5, 0.5};
			species AQI position: {5000+lx * WW * xx_sc + xx, 560 * HH, 0.01} size: {0.5, 0.5};
